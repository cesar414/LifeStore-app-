...
1- Productos clasificados mediante la venta y busqueda
2- Productos por rese;a en el servicio
3- ingresos y ventas
4- Productos no comprados y stock
5- Exit
"""
# Bienvenida!
...
"""
"""
Creacion de diccionarios y funciones
-------------------------------------------------------------------------------------------------------------------------------
"""
...
for par in id_name:
  id = par[0]
  name = par[1]
  name = name.split(' ') #divido el nombre por palabras
  name = ' '.join(name[:5]) #me quedo solo con las 5 palabras
  if id not in Productos_name.keys():
    Productos_name[id] = []
  Productos_name[id].append(name)
...
producto=list(Productos_Score.keys())#transformo Productos_Score en una lista quedandome solo con las llaves
resproducto_list=[]
...
for par in Productos_Score:
  ...
  resproducto_list.append(int(sum(Productos_Score[par])/longscore*100)/100) #saco el promedio de las reseñas 
score_dic={}
...
while not UsuarioMenu:
  ...
  if opcion== '1':
    ...
    #se hace ve si no se vendio de ser el caso se regresa al menu
    if id_product==0:
      ...
      n=0 #contador servira solo para mostrar los lugares
      #imprimo resultados
    ...
    listsuma_precios_mes=[] #lista con la suma de los precios de los productos al mes
    list_meses=[] #lista de los mes ordenados 
    listcont_ventas=[] #lista de ventas por mes
    Productos_utiles=[] #productos que fueron vendidos
    for i in range(12):
      suma_precios_mes=0
      i+=1
      if i>9:
      ...
      #se ingresa datos a la lista correspondiente
    ...
    print(f"las ventas  anual fueron {sum(listcont_ventas)}, teniendo una ganancia de {sum(listsuma_precios_mes)}\n")
    print(f"las ventas promedio anual fueron {int((sum(listcont_ventas)/len(listcont_ventas))*100)/100}\n")
    
    
  elif opcion == '4':
    #saco todos los productos del año que fueron vendidos ese año
    ...
    print(productos_a_eliminar[:15])
    print(productos_a_eliminar[15:30])
    print(productos_a_eliminar[30:45])
    print(productos_a_eliminar[45:])
    print("se recomienda retirar a la brevedad ")
    print("--"*50)
    time.sleep(time_duration)
    Id_stock={}
...
"""
fin del programa donde se vera el menu 
-----------------------------------------------------------------------------------------------------------------------------
"""